...
class Smooth(Algorithm):
    """
    Algorithm from http://en.wikipedia.org/wiki/Smoothsort
    """
    name = "smoothsort"
    ...
    def sort(self, lst):
        ...
        while head < len(lst) - 1:
            if (p & 3) == 3:
                ...
                if self.LP[pshift - 1] >= len(lst) - 1 - head:
        ...
        while pshift != 1 or p != 1:
            if pshift <= 1:
                ...
                self.trinkle(lst, p >> 1, pshift + 1, head - self.LP[pshift] - 1, True)
...
algorithms = [
    ...
    Gnome, Cocktail, OddEven, Comb, Merge, Smooth
]
